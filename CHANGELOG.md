...
- `SkipMcpDiscovery bool` field on `SdkPluginConfig`. When `true`, the CLI loads the plugin's skills, hooks, agents, and commands but skips reading the plugin's `.mcp.json` (passes `--plugin-dir-no-mcp` instead of `--plugin-dir`). Use when the SDK host manages the plugin's MCP connections directly. Port of TypeScript SDK v0.3.172. ([#370](https://github.com/Flohs/claude-agent-sdk-go/issues/370))
