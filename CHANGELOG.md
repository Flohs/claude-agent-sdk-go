...
- `ApiRetryError` string type with constants `ApiRetryErrorOverloaded` (`"overloaded"`) and `ApiRetryErrorRateLimit` (`"rate_limit"`) to distinguish 529 (server overloaded) from 429 (rate limit) retry causes. `ApiRetryMessage` gains an `Error ApiRetryError` field populated from the CLI's `error` key in `api_retry` system messages. Port of TypeScript SDK 0.3.150. ([#299](https://github.com/Flohs/claude-agent-sdk-go/issues/299))
