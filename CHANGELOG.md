...
- `Client.GetUsageExperimental(ctx) (*UsageDataExperimental, error)` returns structured session cost (`TotalCostUSD`), plan rate-limit status (`PlanRateLimit`), and local usage-behavior data (`LocalUsage`) by sending a `get_usage` control request to the running CLI subprocess. **Experimental**: the response schema may change without notice. Port of TypeScript SDK v0.3.169. ([#350](https://github.com/Flohs/claude-agent-sdk-go/issues/350))
