...
- `Client.SetMcpServers(ctx, servers map[string]McpServerConfig) error` dynamically updates the set of MCP servers in the running session via the `mcp_set_servers` control request. Servers not included in the map are removed; servers in the map are added or replaced. Supports all config types: `McpStdioServerConfig`, `McpSSEServerConfig`, `McpHTTPServerConfig`. Port of TypeScript SDK v0.3.163/v0.3.166. ([#341](https://github.com/Flohs/claude-agent-sdk-go/issues/341))
