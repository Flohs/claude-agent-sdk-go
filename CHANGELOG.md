...
- `HookEventMessage` typed struct for hook lifecycle events. When `Options.IncludeHookEvents` is `true`, the CLI emits `hook_started` and `hook_response` system messages for each hook invocation; the SDK now parses these into `*HookEventMessage` (which embeds `SystemMessage` for backward compatibility). Fields: `HookEvent`, `HookID`, `HookName`, `Output` (response only), `ExitCode` (response only), `Outcome` (response only). Port of Python SDK PR anthropics/claude-agent-sdk-python#917. ([#328](https://github.com/Flohs/claude-agent-sdk-go/issues/328))
