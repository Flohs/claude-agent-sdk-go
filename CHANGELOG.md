...
- `ToolUseMetaEntry` struct (fields: `DisplayName string`, `IconURL string`) and `ToolUseMeta map[string]ToolUseMetaEntry` field on `AssistantMessage`. When provided by the CLI, `ToolUseMeta` maps each `tool_use_id` to display-friendly metadata: a human-readable name and optional MCP server icon URL. Nil when not provided. Port of TypeScript SDK v0.3.179 / v0.3.181. ([#396](https://github.com/Flohs/claude-agent-sdk-go/issues/396), [#399](https://github.com/Flohs/claude-agent-sdk-go/issues/399))
