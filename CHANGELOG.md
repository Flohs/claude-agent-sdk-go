...
- `ModelFallbackMessage` typed struct and `ModelFallbackTrigger` type with constants `ModelFallbackTriggerModelNotFound`, `ModelFallbackTriggerPermissionDenied`, `ModelFallbackTriggerOverloaded`, `ModelFallbackTriggerServerError`, `ModelFallbackTriggerLastResort`. The message parser now returns `*ModelFallbackMessage` for `system/model_fallback` events for all fallback triggers. Port of TypeScript SDK v0.3.174. ([#363](https://github.com/Flohs/claude-agent-sdk-go/issues/363))
