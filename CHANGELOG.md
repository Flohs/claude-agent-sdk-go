...
- `ErrorCode string`, `CanUserPurchaseCredits *bool`, and `HasChargeableSavedPaymentMethod *bool` fields on `RateLimitInfo`. These allow SDK consumers to programmatically detect when a user has run out of credits and whether self-service remediation is available. Port of TypeScript SDK v0.3.181. ([#425](https://github.com/Flohs/claude-agent-sdk-go/issues/425))
