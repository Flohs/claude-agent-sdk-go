...
- `Client.SetMcpServers(ctx, servers map[string]*McpSdkServerConfig) error` adds or replaces in-process SDK MCP servers at runtime via the `mcp_set_servers` control request. Servers that declare resources (created via `NewSdkMcpServerWithResources`) have their `resources` capability included in the payload so the CLI correctly calls `resources/list` and `resources/read` on those servers. Fixes a bug where runtime-added resource-bearing servers were invisible to the CLI. Port of TypeScript SDK v0.3.166. ([#349](https://github.com/Flohs/claude-agent-sdk-go/issues/349))
