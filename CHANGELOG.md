...
- `ToolUseMetaEntry` struct and `ToolUseMeta map[string]ToolUseMetaEntry` field on `AssistantMessage`. The optional sidecar is keyed by `tool_use_id` and carries a `DisplayName string` for each tool call, enabling SDK consumers to render human-readable labels instead of raw wire names. Nil when not provided by the CLI. Port of TypeScript SDK v0.3.179. ([#396](https://github.com/Flohs/claude-agent-sdk-go/issues/396))
