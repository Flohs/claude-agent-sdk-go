...
- `CanUseToolFunc` and `Options.CanUseTool` GoDoc updated to clarify that the callback fires **only** when the CLI's permission evaluation resolves to `"ask"` — not for every tool use. Pre-approved tools (via `AllowedTools`, `PermissionMode`, or allow rules) and pre-denied tools (via `DisallowedTools` or deny rules) bypass this callback entirely. Use a `PreToolUse` hook for a universal per-tool interceptor. Port of Python SDK PR anthropics/claude-agent-sdk-python#912. ([#330](https://github.com/Flohs/claude-agent-sdk-go/issues/330))
