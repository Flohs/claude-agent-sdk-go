...
- `ModelClaudeFable5` (`"claude-fable-5"`) and `ModelFable5` (`"fable"`) model name constants for use with `Options.Model`, `AgentDefinition.Model`, and similar fields. Port of TypeScript SDK v0.3.170. ([#427](https://github.com/Flohs/claude-agent-sdk-go/issues/427))
- `ModelFallbackTrigger` string type with constants `ModelFallbackTriggerModelNotFound`, `ModelFallbackTriggerPermissionDenied`, `ModelFallbackTriggerServerError`, `ModelFallbackTriggerLastResort`, and `ModelFallbackTriggerOverloaded`. `ModelFallbackMessage` typed struct (fields: `Trigger`, `OriginalModel`, `FallbackModel`, `UUID`, `SessionID`) emitted as a `system/model_fallback` event when the CLI switches to a fallback model. The `server_error`, `last_resort`, and `overloaded` triggers are new additions beyond the original `model_not_found` and `permission_denied`. Port of TypeScript SDK v0.3.174. ([#427](https://github.com/Flohs/claude-agent-sdk-go/issues/427))
