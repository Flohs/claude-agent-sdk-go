...
- `TaskUpdatedStatus` string type with `"pending"`, `"running"`, `"paused"`, `"completed"`, `"failed"`, `"killed"` values. `TerminalTaskStatuses` variable covering both `task_updated` (`"killed"`) and `task_notification` (`"stopped"`) terminal vocabularies. `TaskUpdatedMessage` typed struct (fields: `TaskID`, `Patch map[string]any`, `Status TaskUpdatedStatus`, `SessionID`, `UUID`) emitted when a background task's state changes; the message parser now returns `*TaskUpdatedMessage` for `system/task_updated` messages. Consumers tracking active tasks should clear them on a terminal status from either `TaskNotificationMessage` or `TaskUpdatedMessage` — a task stopped via `TaskStop` arrives as `task_updated` with `status: "killed"`, not as a `task_notification`. Port of Python SDK PR anthropics/claude-agent-sdk-python#1016. ([#378](https://github.com/Flohs/claude-agent-sdk-go/issues/378))
