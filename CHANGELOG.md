...
- `UsageDataExperimental` struct (`TotalCostUSD *float64`, `PlanRateLimit map[string]any`, `LocalUsage map[string]any`) and `Client.GetUsageExperimental(ctx) (*UsageDataExperimental, error)` method for retrieving session cost, plan rate-limit, and local usage-behavior data from a running session. Backed by the `get_usage` control request. **Experimental: this API may change or be removed without notice.** Port of TypeScript SDK v0.3.169. ([#350](https://github.com/Flohs/claude-agent-sdk-go/issues/350))
