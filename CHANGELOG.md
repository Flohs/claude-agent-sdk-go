...
- Spawn failures on an existing native CLI binary now emit a diagnostic message mentioning a likely libc mismatch (musl binary on a glibc host) and suggest setting `Options.CLIPath` to an alternative. Previously callers saw a cryptic OS error with no context. Port of TypeScript SDK v0.3.178. ([#382](https://github.com/Flohs/claude-agent-sdk-go/issues/382))
