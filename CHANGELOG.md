...
- `AlwaysLoad bool` field on `McpStdioServerConfig`, `McpSSEServerConfig`, and `McpHTTPServerConfig`. When `true`, opts the server back into blocking connection behaviour — the CLI waits for the server to connect before executing the first query. By default (false) MCP servers connect in the background and report `status: "pending"` in the `init` message until ready; `AlwaysLoad: true` is the escape hatch for latency-sensitive servers that must be available on the very first query. Port of TypeScript SDK v0.3.142. ([#272](https://github.com/Flohs/claude-agent-sdk-go/issues/272))
