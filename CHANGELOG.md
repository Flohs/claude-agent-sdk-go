...
- `WorkerShuttingDownMessage` typed struct (fields: `Reason`, `UUID`, `SessionID`) parsed from `system/worker_shutting_down` events emitted by Remote Control workers on graceful exit, enabling remote clients to display the shutdown reason. Port of TypeScript SDK v0.3.178. ([#428](https://github.com/Flohs/claude-agent-sdk-go/issues/428))
