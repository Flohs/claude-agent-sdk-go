...
- `TaskUpdatedStatus` type with `"pending"`, `"running"`, `"paused"`, `"completed"`, `"failed"`, `"killed"` values. `TaskUpdatedMessage` typed struct for `system/task_updated` events, embedding `SystemMessage` with `TaskID`, `Patch`, `Status`, `SessionID`, and `UUID` fields. `TerminalTaskStatuses` map covering both `task_notification` (`"stopped"`) and `task_updated` (`"killed"`) terminal vocabulary. Parser now returns `*TaskUpdatedMessage` for `task_updated` subtypes; parsing never errors regardless of patch shape. Port of Python SDK v0.2.101. ([#378](https://github.com/Flohs/claude-agent-sdk-go/issues/378))
