...
- `WorkerShuttingDownMessage` typed struct for `system/worker_shutting_down` events. Consumers can now programmatically distinguish a graceful Remote Control worker shutdown from an unexpected disconnect. The `Reason` field carries an optional human-readable shutdown explanation. Port of TypeScript SDK v0.3.178. ([#380](https://github.com/Flohs/claude-agent-sdk-go/issues/380))
