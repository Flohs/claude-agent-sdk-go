...
- `Options.ToolConfig *BuiltinToolConfig` for configuring individual built-in tools. The initial supported configuration is `AskUserQuestion.PreviewFormat` which sets the content format for the AskUserQuestion tool's preview field (e.g. `"markdown"`, `"text"`). Forwarded to the CLI via `--tool-config <json>`. Port of TypeScript SDK v0.2.69. ([#343](https://github.com/Flohs/claude-agent-sdk-go/issues/343))
