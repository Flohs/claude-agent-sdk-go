...
- `Client.ApplyFlagSettings(ctx, settings map[string]any)` sends an `apply_flag_settings` control request, allowing the active agent, model, effort, or other session flags to be changed mid-session without restarting. Settings take effect on the next query turn. Pass `nil` as the value for a key to clear that override (e.g. `map[string]any{"agent": nil}` resets to the default agent). Port of TypeScript SDK v0.2.132 / v0.3.161. ([#308](https://github.com/Flohs/claude-agent-sdk-go/issues/308))
