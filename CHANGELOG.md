...
- `ApiRetryError` type with constants `ApiRetryErrorOverloaded` (`"overloaded"`) and `ApiRetryErrorRateLimit` (`"rate_limit"`). The `Error ApiRetryError` field on `ApiRetryMessage` is now populated from the CLI's `error` key, letting callers distinguish server-overloaded (529) retries from rate-limit (429) retries without inspecting `ErrorStatus`. Port of TypeScript SDK v0.3.150. ([#299](https://github.com/Flohs/claude-agent-sdk-go/issues/299))
