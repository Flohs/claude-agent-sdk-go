...
- `ModelFallbackTrigger` type with constants (`ModelFallbackTriggerModelNotFound`, `ModelFallbackTriggerPermissionDenied`, `ModelFallbackTriggerOverloaded`, `ModelFallbackTriggerServerError`, `ModelFallbackTriggerLastResort`) and `ModelFallbackMessage` typed struct for `system/model_fallback` events. Fields: `Trigger`, `FromModel`, `ToModel`. Previously these events arrived as a generic `*SystemMessage`; the typed struct lets consumers react programmatically without inspecting `SystemMessage.Data`. Port of TypeScript SDK v0.3.174. ([#368](https://github.com/Flohs/claude-agent-sdk-go/issues/368))
