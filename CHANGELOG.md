...
- `Client.GetUsageExperimental(ctx) (map[string]any, error)` returns structured session cost, plan rate-limit, and local usage-behaviors data via the `get_usage` control request. The return value schema is unstable and subject to change without notice. Port of TypeScript SDK v0.3.169. ([#342](https://github.com/Flohs/claude-agent-sdk-go/issues/342))
