...
- `Model` type alias (`= string`) and named constants for well-known Claude model IDs and aliases: `ModelFable5` (`"claude-fable-5"`), `ModelOpus48` (`"claude-opus-4-8"`), `ModelSonnet46` (`"claude-sonnet-4-6"`), `ModelHaiku45` (`"claude-haiku-4-5-20251001"`), `ModelFable` (`"fable"`), `ModelOpus` (`"opus"`), `ModelSonnet` (`"sonnet"`), `ModelHaiku` (`"haiku"`). `Model` is a type alias so it is interchangeable with `string` in all existing contexts. Port of TypeScript SDK v0.3.170. ([#369](https://github.com/Flohs/claude-agent-sdk-go/issues/369))
