...
- `TaskUpdatedMessage` typed struct (fields: `TaskID`, `Patch map[string]any`, `Status TaskUpdatedStatus`, `SessionID`, `UUID`) and `TaskUpdatedStatus` type with constants for all lifecycle states (`pending`, `running`, `paused`, `completed`, `failed`, `killed`). `TerminalTaskStatuses` map covers both `task_updated` (`"killed"`) and `task_notification` (`"stopped"`) vocabularies. The message parser now returns `*TaskUpdatedMessage` for `system/task_updated` events. Consumers tracking active task IDs should clear them on terminal status from either `TaskNotificationMessage` or `TaskUpdatedMessage`. Port of Python SDK PR anthropics/claude-agent-sdk-python#1016. ([#367](https://github.com/Flohs/claude-agent-sdk-go/issues/367))
