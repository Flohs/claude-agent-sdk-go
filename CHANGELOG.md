...
- `handleControlRequest` goroutines are now context-aware: when the query context is cancelled while a hook, permission, or MCP callback is executing, the goroutine immediately sends an error response to the CLI instead of waiting for the (potentially blocking) callback to complete. This prevents the CLI subprocess from hanging indefinitely when the SDK caller cancels a query mid-hook. Port of TypeScript SDK 0.3.160. ([#298](https://github.com/Flohs/claude-agent-sdk-go/issues/298))
