...
- `Client.SetMcpServers(ctx, servers map[string]McpServerConfig) error` sends the `mcp_set_servers` control request, allowing callers to add or replace MCP servers in a live session without restarting. Servers omitted from the map are disconnected; builtin servers (e.g. `"claude-in-chrome"`) can be added even if the CLI was launched without them. Port of TypeScript SDK v0.3.163 and v0.3.166. ([#329](https://github.com/Flohs/claude-agent-sdk-go/issues/329))
