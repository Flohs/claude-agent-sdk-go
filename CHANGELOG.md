...
- `Client.ReceiveResponse` no longer stops on the first `ResultMessage` unconditionally. Background-agent results (non-empty `ResultMessage.Origin`) are now forwarded to the caller but do not end the response stream — only the main-session result (`Origin == ""`) stops the stream. Previously a background agent completing before the main turn caused the main turn's result to be silently dropped. Port of TypeScript SDK v0.3.176. ([#375](https://github.com/Flohs/claude-agent-sdk-go/issues/375))
