...
- `HookEventMessage` typed struct for the `hook_started` and `hook_response` system message subtypes emitted when `Options.IncludeHookEvents` is `true`. Fields: `HookEvent`, `HookID`, `HookName` (both subtypes); `Output`, `ExitCode`, `Outcome` (response only). The struct embeds `SystemMessage` for backward compatibility. Port of Python SDK PR anthropics/claude-agent-sdk-python#917. ([#328](https://github.com/Flohs/claude-agent-sdk-go/issues/328))
