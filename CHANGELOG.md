...
- `AlwaysLoad bool` field (JSON key `alwaysLoad`, omitted when false) on `McpStdioServerConfig`, `McpSSEServerConfig`, and `McpHTTPServerConfig`. Starting with CLI v2.1.142, MCP servers connect in the background by default and report `status: "pending"` in the init message until ready. Setting `AlwaysLoad: true` opts a specific server back into blocking behaviour — the CLI waits for that server to connect before the first query is executed. Old CLI versions (< v2.1.142) silently ignore the field. Port of TypeScript SDK v0.3.142. ([#272](https://github.com/Flohs/claude-agent-sdk-go/issues/272))
