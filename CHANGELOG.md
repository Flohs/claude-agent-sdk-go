...
- `Client.GetSettings(ctx context.Context) (map[string]any, error)` sends the `get_settings` control request and returns the effective merged settings for the running session, including runtime-resolved values (e.g. the actual model after alias resolution). Note: the Go SDK already has the offline `ResolveSettings()` function which reads settings from disk without spawning a CLI process; `GetSettings` differs by querying the live CLI subprocess. Port of TypeScript SDK v0.2.72. ([#344](https://github.com/Flohs/claude-agent-sdk-go/issues/344))
