...
- `Client.SetMcpServers(ctx, servers map[string]McpServerConfig) error` for runtime MCP server management. Sends the `mcp_set_servers` control request, replacing the active server set. Servers omitted from the map are disconnected; builtin servers (e.g. `"claude-in-chrome"`) can be added even if the CLI was not started with them. Port of TypeScript SDK v0.3.163 / v0.3.166. ([#329](https://github.com/Flohs/claude-agent-sdk-go/issues/329))
