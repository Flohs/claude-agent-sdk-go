...
- `ErrorCode *string`, `CanUserPurchaseCredits *bool`, and `HasChargeableSavedPaymentMethod *bool` fields on `RateLimitInfo`. These fields are populated when the CLI includes them in `rate_limit_info` events, enabling consumers to detect credit-exhaustion rate limits and determine whether a stored payment method is available for purchasing additional credits. Port of TypeScript SDK v0.3.181. ([#398](https://github.com/Flohs/claude-agent-sdk-go/issues/398))
...
- `AllowedDomains`, `DeniedDomains`, `AllowMachLookup`, and `AllowManagedDomainsOnly` fields on `SandboxNetworkConfig` for fine-grained network domain control in sandboxed environments. Port of TypeScript SDK v0.3.144. ([#182](https://github.com/Flohs/claude-agent-sdk-go/issues/182))
