...
- `RateLimitInfo` gains three new fields from TypeScript SDK v0.3.181: `ErrorCode *string` (set when rate-limit is due to credit exhaustion), `CanUserPurchaseCredits *bool`, and `HasChargeableSavedPaymentMethod *bool`. These enable UX flows like "buy more credits" when the user hits a credits-exhausted rate limit. ([#398](https://github.com/Flohs/claude-agent-sdk-go/issues/398))
