...
- `SkipMcpDiscovery bool` field (`json:"skipMcpDiscovery,omitempty"`) on `SdkPluginConfig`. When `true`, the CLI loads the plugin directory without reading its `.mcp.json` or manifest `mcpServers`, passing `--plugin-dir-no-mcp` instead of `--plugin-dir`. Use this when the SDK host owns the plugin's MCP connections and wants to avoid duplicate registration. Requires CLI >= v2.1.172. Port of TypeScript SDK v0.3.172. ([#426](https://github.com/Flohs/claude-agent-sdk-go/issues/426))
