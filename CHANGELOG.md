...
- `SkipMcpDiscovery bool` field on `SdkPluginConfig`. When `true`, the SDK passes `--plugin-dir-no-mcp` to the CLI instead of `--plugin-dir`, causing the CLI to load skills, hooks, agents, and commands from the plugin path without reading its `.mcp.json` or `manifest.mcpServers`. Useful when the SDK host manages the plugin's MCP connections itself and wants to avoid duplicate registration. Requires CLI >= v2.1.172. Port of TypeScript SDK v0.3.172. ([#370](https://github.com/Flohs/claude-agent-sdk-go/issues/370))
