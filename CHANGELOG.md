...
- `SkipMcpDiscovery bool` field on `SdkPluginConfig`. When `true`, the CLI loads skills/hooks/agents/commands from the plugin path without re-reading its `.mcp.json`, using `--plugin-dir-no-mcp` instead of `--plugin-dir`. Use when the SDK host manages the plugin's MCP connections independently. Port of TypeScript SDK v0.3.172. ([#354](https://github.com/Flohs/claude-agent-sdk-go/issues/354))
