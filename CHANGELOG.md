...
- `TaskUpdatedMessage` typed struct for `system/task_updated` events, `TaskUpdatedStatus` type with constants (`pending`, `in_progress`, `completed`, `failed`, `stopped`, `deleted`), and `TerminalTaskStatuses` map for bookkeeping active background tasks. Consumers tracking active tasks can check `TerminalTaskStatuses[msg.Status]` to avoid hanging when a task finishes via a `task_updated` message without a corresponding `TaskNotificationMessage`. Port of Python SDK v0.2.101. ([#423](https://github.com/Flohs/claude-agent-sdk-go/issues/423))
