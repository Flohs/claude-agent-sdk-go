...
- `Client.GetSettings(ctx) (map[string]any, error)` returns the effective merged settings for the running session via the `get_settings` control request. The response includes an `"applied"` section with runtime-resolved values (e.g. the actual model and effort level after alias resolution). Distinct from the offline `ResolveSettings()` function which reads from disk. Port of TypeScript SDK v0.2.72. ([#344](https://github.com/Flohs/claude-agent-sdk-go/issues/344))
