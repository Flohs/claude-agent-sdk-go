...
- `ToolUseMetaEntry` struct (fields: `DisplayName string`, `IconURL string`) and `ToolUseMeta map[string]ToolUseMetaEntry` field on `AssistantMessage`. When the CLI attaches a `tool_use_meta` sidecar to an assistant message, SDK consumers can look up a tool call's human-readable label and icon URL by tool-use ID. Port of TypeScript SDK v0.3.179 / v0.3.181. ([#424](https://github.com/Flohs/claude-agent-sdk-go/issues/424))
