...
- Model string constants in a new `models.go` file: `ModelFable5` (`"claude-fable-5"`), `ModelFable` (`"fable"`), `ModelOpus48` (`"claude-opus-4-8"`), `ModelOpus` (`"opus"`), `ModelSonnet46` (`"claude-sonnet-4-6"`), `ModelSonnet` (`"sonnet"`), `ModelHaiku45` (`"claude-haiku-4-5-20251001"`), `ModelHaiku` (`"haiku"`). Use with `Options.Model` and `Options.FallbackModel` to avoid typos and enable autocomplete. Port of TypeScript SDK v0.3.170. ([#413](https://github.com/Flohs/claude-agent-sdk-go/issues/413))
