...
//
// CanUseToolFunc is NOT a universal tool interceptor: it does not fire for
// tools that are pre-approved or pre-denied. Use a PreToolUse hook to observe
// or gate every tool call regardless of permission settings.
//
// ToolPermissionContext.ToolUseID is always non-empty when delivered via
// CanUseTool. Port of Python SDK PR anthropics/claude-agent-sdk-python#912.
